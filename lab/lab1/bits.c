...
int bitRepeat(int x, int n) {
  int modulo = n%32;
  int is32 = (!modulo << 31) >> 31;
  int mask = is32 | ~(-1 << modulo);
  int result = x & mask;
...
int isLessOrEqual(int x, int y) {
  ...
  int yIsGreater = ~(diff >> 31);
  return !!((bothDifferent & xIsNegative) | (~bothDifferent & yIsGreater));
}
/* 
...
int logicalShift(int x, int n) {
  ...
  int mask;
  int negOne = (~1 + 1);
  offset = (~isZero & offset) | (isZero & 0);
  mask = (1 << offset) + negOne;
  mask = (~isZero & mask) | (isZero & negOne);
  return xsra & mask;
}
/* 
